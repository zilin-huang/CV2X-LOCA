...
T=1;            % 采样周期
load('D:\MATLAB\R2016b\bin\7. TITS\Test1_定位性能\车速7-衰减1-噪声1\GRNN\distance_GRNN.mat')
N= length(distance_GRNN);        % 采样次数
...
X(:,1)=[distance_GRNN_T(1,1),distance_GRNN_T(2,1)-distance_GRNN_T(1,1),distance_GRNN_T(1,2),distance_GRNN_T(2,2)-distance_GRNN_T(1,2)];    % 目标初始位置、速度     4*60 第一列是初始位置
...
error_GRNN_UKF=sqrt(sum((distance_GRNN_UKF(1:2,:)-trace_1').^2))./2   %误差

diff = abs(distance_GRNN_UKF(1:2,:)-trace_1')  % 预测值与真实值的差距
error_GRNN_UKF_high_x = max(max(diff(1,:)))   % x轴最大误差
error_GRNN_UKF_low_x = min(min(diff(1,:)))   % x轴最小误差
error_GRNN_UKF_high_y = max(max(diff(2,:)))   % y轴最大误差
error_GRNN_UKF_low_y = min(min(diff(2,:)))   % y轴最小误差

mean_error_GRNN_UKF=mean(error_GRNN_UKF)            %定位误差

rmse_error_GRNN_UKF=(sqrt(mean((distance_GRNN_UKF(1,:)-trace_1(:,1)').^2))+sqrt(mean((distance_GRNN_UKF(2,:)-trace_1(:,2)').^2)))/2  % RMSE
rmse_error_GRNN_UKF_high_x=sqrt(mean((distance_GRNN_UKF(1,:)-trace_1(:,1)').^2))   % x轴的RMSE
rmse_error_GRNN_UKF_high_y=sqrt(mean((distance_GRNN_UKF(2,:)-trace_1(:,2)').^2))   % y轴的RMSE
...
clc;
clear;
load('D:\MATLAB\R2016b\bin\7. TITS\Test1_定位性能\车速7-衰减1-噪声1\GRNN_UKF\totall_error.mat')
