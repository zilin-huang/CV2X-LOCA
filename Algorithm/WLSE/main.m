%%
% 本Test文档的目的：选取每个轨迹点最近的4个AP数据,并应用加权最小二乘法（WLSM）
% 注意：N取最近4个点
...
diff = abs(distance_WLSE(1:2,:)-trace_1')  % 预测值与真实值的差距
error_WLSE_high_x = max(max(diff(1,:)))   % x轴最大误差
error_WLSE_low_x = min(min(diff(1,:)))   % x轴最小误差
error_WLSE_high_y = max(max(diff(2,:)))   % y轴最大误差
error_WLSE_low_y = min(min(diff(2,:)))   % y轴最小误差

mean_error_WLSE=mean(error_WLSE)            %定位误差

rmse_error_WLSE=(sqrt(mean((distance_WLSE(1,:)-trace_1(:,1)').^2))+sqrt(mean((distance_WLSE(2,:)-trace_1(:,2)').^2)))/2  % RMSE
rmse_error_WLSE_high_x=sqrt(mean((distance_WLSE(1,:)-trace_1(:,1)').^2))   % x轴的RMSE
rmse_error_WLSE_high_y=sqrt(mean((distance_WLSE(2,:)-trace_1(:,2)').^2))   % y轴的RMSE
...
clc;
clear;
load('D:\MATLAB\R2016b\bin\7. TITS\Test1_定位性能\车速7-衰减1-噪声1\WLSE\totall_error.mat')