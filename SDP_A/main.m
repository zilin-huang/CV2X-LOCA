...
error_SDP_A=sqrt(sum((distance_SDP_A(1:2,:)-trace_1').^2))./2   %误差

diff = abs(distance_SDP_A(1:2,:)-trace_1')  % 预测值与真实值的差距
error_SDP_A_high_x = max(max(diff(1,:)))   % x轴最大误差
error_SDP_A_low_x = min(min(diff(1,:)))   % x轴最小误差
error_SDP_A_high_y = max(max(diff(2,:)))   % y轴最大误差
error_SDP_A_low_y = min(min(diff(2,:)))   % y轴最小误差

mean_error_SDP_A=mean(error_SDP_A)            %定位误差

rmse_error_SDP_A=(sqrt(mean((distance_SDP_A(1,:)-trace_1(:,1)').^2))+sqrt(mean((distance_SDP_A(2,:)-trace_1(:,2)').^2)))/2  % RMSE
rmse_error_SDP_A_high_x=sqrt(mean((distance_SDP_A(1,:)-trace_1(:,1)').^2))   % x轴的RMSE
rmse_error_SDP_A_high_y=sqrt(mean((distance_SDP_A(2,:)-trace_1(:,2)').^2))   % y轴的RMSE
...
clc;
clear;
load('D:\MATLAB\R2016b\bin\7. TITS\Test1_定位性能\车速7-衰减1-噪声1\SDP_A\totall_error.mat')
